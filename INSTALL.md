...
Contract version: 3. Candidate consumer plugin version: 1.2.77. Published installable native version: 1.2.75.
...
> 
> 1. Read-only review. Do not run setup commands or change configuration yet. Verify by read-only inspection that the exact source exists: https://github.com/Invompt/invompt-plugin at published tag v1.2.75. Candidate v1.2.77 is unpublished and must not be used or replaced with main, latest, another tag, another host, or another route. If the published source is missing or this host cannot provide native OAuth for this connection, stop without changes.
> 
> 2. Concrete approval. Before changing anything, show me this summary:
> - Source: https://github.com/Invompt/invompt-plugin at published tag v1.2.75
> - Proposed configuration: User-scoped native plugin plus the host OAuth entry for the Invompt MCP connection.
> - Permissions: The host requests the listed Invompt OAuth scopes; no static credential is added.
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
> 1. claude plugin marketplace add https://github.com/Invompt/invompt-plugin.git#v1.2.75
...
> - Permissions: The host requests the listed Invompt OAuth scopes; no API key or static header is added.
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
> 1. codex plugin marketplace add Invompt/invompt-plugin --ref v1.2.75
> 2. codex plugin add invompt@invompt
> - Exact OAuth start: codex mcp login invompt --scopes invoices:read,invoices:write,invoices:update,invoices:archive,clients:read,clients:write,settings:read,settings:write,templates:read,templates:write
...
> 
> 4. User-owned OAuth. Start OAuth only with this step: codex mcp login invompt --scopes invoices:read,invoices:write,invoices:update,invoices:archive,clients:read,clients:write,settings:read,settings:write,templates:read,templates:write. Hand the browser flow to me. Never request or accept authorization codes, passwords, MFA values, cookies, API keys, access tokens, or refresh tokens. Wait for me to report that the browser flow finished; opening a URL is not success.
> 
...
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
> 1. /plugins install https://github.com/Invompt/invompt-plugin/releases/tag/v1.2.75
> 2. /plugins reload
...
> 
> 1. Read-only review. Do not run setup commands or change configuration yet. Verify by read-only inspection that the exact source exists: https://mcp.invompt.com/mcp. Candidate v1.2.77 is unpublished and must not be used or replaced with main, latest, another tag, another host, or another route. If the published source is missing or this host cannot provide native OAuth for this connection, stop without changes.
> 
> 2. Concrete approval. Before changing anything, show me this summary:
...
> - Permissions: The host requests the listed Invompt OAuth scopes through its connector flow.
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
...
> - Permissions: The host requests the listed Invompt OAuth scopes through its native login.
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
...
> - Source: https://mcp.invompt.com/mcp
> - Proposed configuration: One remote HTTP MCP entry named invompt; no local command or loopback server.
> - Permissions: The host requests the listed Invompt OAuth scopes when supported by the installed version.
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
...
> - Permissions: The host requests the listed Invompt OAuth scopes when its installed version supports them.
> - OAuth scopes: invoices:read, invoices:write, invoices:update, invoices:archive, clients:read, clients:write, settings:read, settings:write, templates:read, templates:write
> - Configuration steps:
...
- The only requested permissions are: `invoices:read`, `invoices:write`, `invoices:update`, `invoices:archive`, `clients:read`, `clients:write`, `settings:read`, `settings:write`, `templates:read`, `templates:write`.
